...
export const crearUsuario = async (req, res, next) => {
  try {
    ...
    const passwordHashed = bcrypt.hashSync(password, 12).replace("$2b", "$2y");
...
export const obtenerUsuarios = async (req, res, next) => {
  try {
    const usuarios = await Usuario.findAll({
      attributes: ["id", "name", "email", "password", "id_rol", "estado"],
      include: {
        model: rol,
        attributes: ["nombre"],
        as: "Rol",
...
export const actualizarUsuario = async (req, res, next) => {
  try {
    ...
    if (usuario) {
      if (password) {
        usuario.password = bcrypt.hashSync(password, 12).replace("$2b", "$2y");
      }
